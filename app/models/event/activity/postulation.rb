...
class Event::Activity::Postulation < ApplicationRecord
  ...
  ACTIVITY_TYPES = %w[workshop speech other].freeze
  ACTIVITY_DIFFICULTY_LEVELS = %w[everyone easy medium hard expert].freeze
  ...
  def approve
    ...
    activity = Event::Activity.create(
      ...
      event_day: event_days.first,
      title: activity_title,
      description: activity_description,
      postulation: self
    )

    email = ::Email.find_by(address: postulant_email)
    if email.present?
      participant = email.emailable
    else
      participant = Entity::Person.create(
        firstname: postulant_firstname,
        lastname: postulant_lastname,
        bio: postulant_bio,
        user: User.create(email: postulant_email)
      )
      participant.emails.create(address: postulant_email)
    end
