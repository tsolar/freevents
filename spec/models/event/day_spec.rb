  ...
  describe "#to_s" do
    context "when starts_at is present" do
      it "returns formatted start_date" do
        event_day = create(:event_day)
        expect(event_day.to_s).to eq "#{I18n.l(event_day.starts_at, format: :event_day_with_time)} - #{I18n.l(event_day.ends_at, format: :only_time)}"
      end
      ...
      context "when ends_at next day" do
        it "returns formatted including start date and end date" do
          ...
          expect(event_day.to_s).to eq "#{I18n.l(event_day.starts_at, format: :event_day_with_time)} - #{I18n.l(event_day.ends_at, format: :event_day_with_time)}"
        end
      end
    end
